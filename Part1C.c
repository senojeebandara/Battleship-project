...
int main() {
   ...
   //storing the x and y coordinates in the array
    ...
    switch (type){
        ...
        case 2:
                ...
                fprintf(escort,"                            Minimum Velocity: %d, Maximum Velocity - %d\n", Vmin, Vmax);
                fprintf(escort,"                            Minimum Angle: %d, Maximum Angle - %d\n", min_ang_e, max_ang_e);
                eship[j-1].impactPower = 0.06 *100;
                printf("\n");
            ...
            case 3:
                    ...
                    eship[j-1].impactPower = 0.07 *100;
                    printf("\n");
                    break;
                ...
                case 4:
                        ...
                        fprintf(escort,"                            Minimum Velocity: %d, Maximum Velocity - %d\n", Vmin, Vmax);
                        fprintf(escort,"                            Minimum Angle: %d, Maximum Angle - %d\n", min_ang_e, max_ang_e);
                        eship[j-1].impactPower = 0.05 * 100;
                        printf("\n");
                        break;
  ...
  if (distance <= Rmax && distance >= Rmin) {
    ...
    while(battleshipHealth > 0){

    struct escortships eship[Escortships];
    battleshipHealth -= eship[j-1].impactPower;
    printf("Battleship Health: %d\n", battleshipHealth);
...
break;
     ...
     const int theta_min = 10;
     float angle_of_elevation = atan((float)(y - y_battle) / (float)(x - x_battle));
     if (angle_of_elevation < (theta_min - 90) * PI/180){
        angle_of_elevation = (theta_min - 90) * PI/180;
     } else if(angle_of_elevation > 90 * PI/180){
        angle_of_elevation = 90 * PI/180;
        }
    if (angle_of_elevation >= (theta_min - 90) * PI/180 && angle_of_elevation <= 90*PI/180){
