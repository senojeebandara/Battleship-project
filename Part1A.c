...
int main() {
    ...
    while(j<=Escortships && !sankBattleship){
    ...
    //minimum and maximum angle of the escort ship shell
    int min_ang_e;
    int max_ang_e;

    //minimum and maximum velocity of the escort ship shell
    int Vmin;
    int Vmax;


    //choosing the escort ship type
    int type = random(1, 5);
    switch (type){
        case 1:
            printf("Esortship %d: \n", j);
    ...
    // storing the escort ship details in the array
    ...
    for (int count = 0; count < Escortships; count++) {
     //Calculate the distance between escortship and battleship
   ...
   if (distance <= Rmax && distance >= Rmin) {
    ...
    saveBattlefieldConditions(Escortships, x_battle, y_battle, canva_1, canva_2);
     ...
     printf("Escortship %d doesnt sink the battleship.\n\n", count+1);
     ...
     total_time = total_time +time_to_hit;
